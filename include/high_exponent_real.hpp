...
namespace numbers
{
  ...
  */
  ...
  inline double rshift(double d, int n)
  {
    return d * std::pow(2,-n);
  }

  template <std::floating_point D, typename E, bool Norm>
  double to_double(const high_exponent_real<D, E, Norm> &from) { return from.to_double(); }

  template <std::floating_point D, typename E, bool Norm>
  std::pair<D, E>
  mantissa_exponent(const high_exponent_real<D, E, Norm> &from) { return {from.mantissa(), from.exponent()}; }

  // inline std::pair<double,int> mantissa_exponent(double d) { return {d,0}; }

  template <std::floating_point D, typename E, bool N, real From>
  struct number_cast_t<high_exponent_real<D, E, N>, From>
  {
    static high_exponent_real<D, E, N> cast(const From &from)
    {
      auto p = mantissa_exponent(from);
      return {p.first, p.second};
    }
  };

  template <std::floating_point D, typename E, bool Norm>
  struct number_traits<high_exponent_real<D, E, Norm>>
  {
...
static_assert(numbers::real<numbers::high_exponent_double>);
static_assert(numbers::non_native_floating_point<numbers::high_exponent_double>);