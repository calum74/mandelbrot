...
struct shader_parameters {
  int colour_scheme = 138; // Any integer used to seed the random number generator
  double colour_gradient = 30; // Iterations per colour
  double colour_offset = 0;
  bool auto_gradient = true;

  bool shading = true;
  double ambient_brightness = 0.6;       // Value between 0 and 1
  double source_brightness = 1.2;        // Value between 0 and 1, or higher for saturation
